...
class DC_EWL_and_CE_loss(nn.Module):
    def __init__(self, soft_dice_kwargs, ewl_kwargs, ce_kwargs, weight_ce=1, weight_dice=1, weight_ew=1, 
        ...
        :param weight_ew:
        """
        super(DC_EWL_and_CE_loss, self).__init__()
        if ignore_label is not None:
            ce_kwargs['ignore_index'] = ignore_label
    ...
    def forward(self, net_output: torch.Tensor, target: torch.Tensor, ewtr = None):
        """
        target must be b, c, x, y(, z) with c=1
        :param net_output:
        :param target:
        :param ewtr: Euclidean weighted transform for error-based loss (optional)
        :return:
        """
        ...
        # sanitize non-finite values before combining losses
        if isinstance(dc_loss, torch.Tensor):
            dc_loss = torch.nan_to_num(dc_loss, nan=0.0, posinf=1e3, neginf=-1e3)
        if isinstance(ce_loss, torch.Tensor):
            ce_loss = torch.nan_to_num(ce_loss, nan=0.0, posinf=1e3, neginf=-1e3)
